...
subst old sub (Application ((Lambda idConst exp):xs))
    |   idConst == old                  =   (Application ((Lambda idConst exp):xs))
    |   otherwise                       =   (Application ((Lambda idConst exp):(map (subst old sub) xs)))
